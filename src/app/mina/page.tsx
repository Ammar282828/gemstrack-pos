...
interface Payment {
  ...
  amount: number;
  date: Date;
  category: LedgerCategory; // 'equity' (capital contribution) or 'loan' (lent to business)
}
...
export default function MinaAccountPage() {
    ...
    try {
      ...
      for (const d of snap.docs) {
        ...
        const row: Payment = {
          ...
          // Historical entries without a category default to 'equity' — they were
          // logged before the loan vs equity distinction existed.
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setAmmarBuckets({ equityIn, loanIn, equityOut, loanOut });
      } catch { /* silent — calculator just shows Mina-only if Ammar's ledger is unavailable */ }
    })();
  ...
  const balances = partnerBalance(buckets, minaExpShare, minaRevShare);

  // Other partner (Ammar) — both partners share equally in P&L (50/50).
  const ammarBalances = partnerBalance(ammarBuckets, minaExpShare, minaRevShare);
  ...
  // Positive = Mina owes business, Negative = business owes Mina (legacy formula
  // retained for the historic top-line indicator). totalClaim from balances is
  // the new authoritative "net owed to Mina" number (same magnitude, opposite sign).
    ...
    <div className="container mx-auto py-6 px-4 max-w-3xl space-y-5">
      ...
      {isLoading ? (
        ...
        <>
          ...
          {(() => {
            const claim = balances.totalClaim; // positive = biz owes Mina
            const positive = claim > 0;
            const headline = positive ? 'Business owes Mina' : claim < 0 ? 'Mina owes the business' : 'All settled';
                ...
                <CardContent className="pt-6 pb-5 space-y-4">
                  ...
                  <div className="border-t pt-3 space-y-1.5 text-sm">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Loan she gave the business</span>
                      <span className="tabular-nums text-success">+ {fmt(balances.loanBalance)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Her equity stake (capital in)</span>
                      <span className="tabular-nums text-success">+ {fmt(balances.equityBalance)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Her share of the {balances.netPnL >= 0 ? 'profit' : 'loss'}</span>
                    ...
                    <div className="flex justify-between font-semibold">
                      <span>{positive ? 'Net owed to Mina' : claim < 0 ? 'Net Mina owes' : 'Net'}</span>
          ...
          {/* ── Show working — auto-computed expense/revenue shares + calculator ── */}
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                <Wallet className="w-4 h-4 text-success" /> Payments by Mina
                {payments.length > 0 && (
