/**
 * HEIC → JPEG for any photo action (src/lib/photo-file.ts `readablePhoto`).
 *
 * A photo picked from the Photos library in Chrome on a Mac arrives as HEIC, which Chrome
 * cannot draw; the page sends it here and uses the JPEG that comes back. Both shops — unlike
 * /api/website/post/convert, which belongs to Post a Piece and exists only where that does.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.warn('[photo convert] conversion failed:', e instanceof Error ? e.message : e);
