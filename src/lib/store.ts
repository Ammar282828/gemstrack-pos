...
const initialProducts: Product[] = [
  ...
  {
    sku: "NEC-000001", name: "Necklaces - NEC-000001", categoryId: "cat2", metalWeightG: 12.5, stoneWeightCt: 1.5, wastagePercentage: 12,
    makingRatePerG: 1200, stoneRatePerCt: 75000, miscCharges: 1500, imageUrl: "https://placehold.co/300x300.png"
  },
  {
    sku: "EAR-000001", name: "Earrings - EAR-000001", categoryId: "cat3", metalWeightG: 3.0, stoneWeightCt: 0.75, wastagePercentage: 8,
  ...
  {
    sku: "BRA-000001", name: "Bracelets - BRA-000001", categoryId: "cat4", metalWeightG: 8.0, stoneWeightCt: 0, wastagePercentage: 10,
...
export const useAppStore = create<AppState>()(
  persist(
    immer((set, get) => ({
      ...
      addProduct: (productData) => {
        ...
        set((state) => {
          ...
          newProduct = {
            ...
            sku: generatedSku,
            // qrCodeDataUrl: '', // qrCodeDataUrl is optional and can be undefined
          };
      ...
      updateProduct: (sku, updatedFields) =>
        set((state) => {
          ...
          if (productIndex !== -1) {
            // Ensure name and sku are not overwritten if they are part of updatedFields
    ...
    {
      name: 'gemstrack-pos-storage', // The key used in localStorage
      storage: createJSONStorage(() => {
        // Conditionally return a dummy storage for SSR, localStorage for client
      ...
      onRehydrateStorage: (_state, error) => {
        ...
        // Ensure setHasHydrated is called after the store is initialized and potentially rehydrated
      ...
      partialize: (state) => {
        ...
        // Exclude _hasHydrated as it's a runtime flag
        const {
          _hasHydrated, // explicitly exclude
          ...rest // include all other state properties
        } = state;
        return rest;
      },
...
export const selectCartDetails = (state: AppState) => {
  return state.cart.map(cartItem => {
    ...
    if (!product) return null; // Should ideally not happen if cart is managed well
    const costs = calculateProductCosts(product, state.settings.goldRatePerGram);
    return {
      ...product,
      ...costs,
      quantity: cartItem.quantity,
      lineItemTotal: costs.totalPrice * cartItem.quantity, // total for this line item
    };
...
// Hook to determine if the store has been rehydrated
export const useIsStoreHydrated = () => {
  ...
  React.useEffect(() => {
    // Function to sync local state with store state
    const syncHydrationState = () => {
      const storeHydrated = useAppStore.getState()._hasHydrated;
      // console.log('[GemsTrack] useIsStoreHydrated Effect: Store hydrated state is', storeHydrated);
    ...
    // Subscribe to changes in the store's _hasHydrated flag
    const unsubscribe = useAppStore.subscribe(
      (state) => state._hasHydrated, // Selector: only listen to changes in _hasHydrated
      (hydrated) => { // Listener: called when the selected state changes
  ...
  // console.log('[GemsTrack] useIsStoreHydrated: Hook rendering, isHydrated =', isHydrated);
  return isHydrated;
};
