  ...
  persist(
    ...
    })),
    {
      ...
      partialize: (state) => {
        ...
        return { settings, categories, products, customers, cart, generatedInvoices };
      },
      version: 1,
    }
  )
...
export const useIsStoreHydrated = () => {
  ...
  React.useEffect(() => {
    const syncHydrated = () => setIsHydrated(useAppStore.getState()._hasHydrated);
    ...
    const unsubscribe = useAppStore.subscribe(
      ...
      (hydrated) => {
        // console.log('[Taheri POS] useIsStoreHydrated: Subscription received _hasHydrated =', hydrated);
