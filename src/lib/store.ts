  ...
  persist(
    immer((set, get) => ({
      ...
      addProduct: async (productData) => {
        ...
        if (!productData.isCustomPrice && !autoGeneratedName) {
            autoGeneratedName = `${category.title} - ${generatedSku}`;
        } else if (productData.isCustomPrice) {
        ...
        // Ensure karat is set for gold, and unset for other metals
        if (finalProductData.metalType === 'gold') {
          if (!finalProductData.karat) {
            finalProductData.karat = '21k'; // Default to 21k if not provided
          }
        } else {
          delete finalProductData.karat;
        }

        if (finalProductData.secondaryMetalType !== 'gold') { 
          delete finalProductData.secondaryMetalKarat; 
        }

        const newProduct: Product = { ...finalProductData, name: autoGeneratedName, sku: generatedSku } as Product;
