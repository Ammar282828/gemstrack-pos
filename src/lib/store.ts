...
export interface InvoiceItem extends Product { // Inherits name from Product
  quantity: number;
...
// ProductDataForAdd now excludes 'name' as it will be auto-generated
type ProductDataForAdd = Omit<Product, 'sku' | 'qrCodeDataUrl' | 'name'>;
// ProductDataForUpdate now excludes 'name' as it's not updatable via form
...
export const useAppStore = create<AppState>()(
  persist(
    immer((set, get) => ({
      ...
      addProduct: (productData) => { // productData is now ProductDataForAdd (no 'name')
        ...
        set((state) => {
          ...
          newProduct = {
            ...productData, // productData now doesn't have name
            name: autoGeneratedName, // Add the auto-generated name
            sku: generatedSku,
            qrCodeDataUrl: '', 
          };
      ...
      updateProduct: (sku, updatedFields) => // updatedFields is ProductDataForUpdate (no 'name')
        set((state) => {
          const productIndex = state.products.findIndex((p) => p.sku === sku);
          if (productIndex !== -1) {
            // Ensure SKU and name are not part of updatedFields or are ignored
      ...
      generateInvoice: (customerId, invoiceGoldRate, discountAmount) => {
        ...
        const invoiceItems: InvoiceItem[] = cart.map(cartItem => {
          ...
          return {
            ...product, // product already includes its name
...
export const useIsStoreHydrated = () => {
  const isHydrated = useAppStore(
    React.useCallback((state: AppState) => state._hasHydrated, [])
  );
  return isHydrated;
};
